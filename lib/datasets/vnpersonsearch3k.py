...
class VNpersonsearch3k(BaseDataset):
    def __init__(self,
                 root,
                 list_path=None,
                 num_samples=None,
        ...
        super(VNpersonsearch3k, self).__init__(ignore_label, base_size,
                                  crop_size, downsample_rate, scale_factor, mean, std)
        self.root = root
        self.num_classes = num_classes
        self.list_path = os.path.join(self.root, 'vn_person_search_3k/vn3k.json')
    ...
    def save_pred(self, preds, sv_path, name):
        ...
        for i in range(preds.shape[0]):
            ...
            dataset_name, file_name = '/'.join(name[i].split('/')[:-1]), name[i].split('/')[-1]
